...
BOOL CDlgAffineWarp::OnInitDialog()
{
	...
	// TODO:  在此添加额外的初始化
	UpdateData(true);
	m_comboBox1.AddString(_T("LINEAR"));
	m_comboBox1.AddString(_T("NEAREST"));
	m_comboBox1.AddString(_T("AREA"));
	m_comboBox1.AddString(_T("CUBIC"));
	m_comboBox1.AddString(_T("LANCZOS4"));
	m_comboBox1.AddString(_T("FILL_OUTLIERS"));
	m_comboBox1.AddString(_T("INVERSE_MAP"));
	m_comboBox1.SetCurSel(0);
	
	m_comboBox2.AddString(_T("正方向"));
	m_comboBox2.AddString(_T("反方向"));
...
void CDlgAffineWarp::OnBnClickedRadio2()
{
	// TODO: 在此添加控件通知处理程序代码
	UpdateData(true);
	//m_pointSelect = m_radio.GetCheck();
	//m_radio.SetCheck(m_pointSelect);
	UpdateData(false);
}
...
//HBRUSH CDlgAffineWarp::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
//{
...
//	if(nCtlColor == CTLCOLOR_DLG)	return (HBRUSH)brush.GetSafeHandle();
//	// TODO: GetSafeHandle();另一个画笔
//	return hbr;
//}
